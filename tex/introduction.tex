...
\item\index{\texttt{f.:A}!Mizar for $f(A)$} Applying a function to a set is denoted $f(A)$, but in Mizar it is
  denoted ``\texttt{f.:A}''
\item\index{{\texttt{f"\""}}!Mizar for $f^{-1}$} The inverse function relating $y=f(x)$ to $f^{-1}(y)=x$, in Mizar
  is denoted by ``\texttt{f".y}''. More generally, instead of using
  superscript $-1$, Mizar will use double quotes.
\item\index{{\texttt{f"|"A}}!Mizar for $f\vert_{A}$} If $f\colon X\to Y$ is a function of sets, and $A\subset X$, then
...
\begin{remark}[Mizar idiom]\index{Thesis@\texttt{thesis}}
...
``\verb#GROUP_1:def 5#''.\mizindex{\texttt{GROUP\_1}!def 05@\texttt{def 5}}
A formal ``\verb#inverse_op(G)#'' is given in
``\verb#GROUP_1:def 6#''.\mizindex{\texttt{GROUP\_1}!def 06@\texttt{def 6}}
...
notation in \verb#GROUP_2:def 1#\mizindex{\texttt{GROUP\_2}!def 01@\texttt{def 1}}
\end{remark}

\N{Power operator}
For any $g\in G$, and $n\in\NN_{0}$, we often write $g^{n}$ with the
understanding this stands for $g^{0}=1_{G}$ and $g^{n+1}=g^{n}\cdot g$.
This is defined in ``\verb#GROUP_1:def 7#''
\mizindex{\texttt{GROUP\_1}!def 07@\texttt{def 7}}. But we generalize this in
mathematics to let $n\in\ZZ$, allowing the case when $n<0$ to be defined
by $g^{n} = (g^{|n|})^{-1}$.

Mizar does this in
``\verb#GROUP_1:def 8#'',\mizindex{\texttt{GROUP\_1}!def 08@\texttt{def 8}} following the conventions
taken by earlier articles in Mizar that $x^{y}$ is denoted by
``\verb#x |^ y#''.\index{{\texttt{x "|"\Caret\ y}}!Mizar for $x^{y}$}
For the sake of consistency, Mizar explicitly states in
``\verb#GROUP_1:def 8#'' when $g\in G$ and $n{:}\NN_{0}$, it follows
...
``\verb#GROUP_1:def 10#''\mizindex{\texttt{GROUP\_1}!def 10@\texttt{def 10}} as ``\verb#being_of_order_0#''\index{Being of Order 0@\texttt{being\_of\_order\_0}}.
...
``\verb#ord g#''\index{Order!\texttt{ord g}} in ``\verb#GROUP_1:def 11#''.\mizindex{\texttt{GROUP\_1}!def 11@\texttt{def 11}}
...
``\verb#card G#''\index{Cardinality@\texttt{card G}} be the cardinality
of the underlying set.

\N{Abelian Groups}
Sadly, Mizar departs from mathematical practice, and defines an
attribute in ``\verb#GROUP_1:def 12#''\mizindex{\texttt{GROUP\_1}!def 12@\texttt{def 12}} for a \verb#multMagma# to be
``\verb#commutative#''.\index{Commutative@\texttt{commutative}}\index{Abelian|see{\texttt{commutative}}}
