...
proof
  ...
  set Q = (phi .: P);
  A3: P,Q are_isomorphic by \nwlinkedidentc{Th20}{NW1Nnu7l-3WScXE-1}, GRSOLV_1:def 3;
...
proof
  ...
  (id the carrier of G).x = x by FUNCT_1:17
                         .= x |^ 1_G by GROUP_3:19;
  hence thesis;
end;
...
proof
  ...
  set h = [.x,y.];
  x in G & y in G;
  then B3: h in commutators G by GROUP_5:58;
  dom phi = the carrier of G by FUNCT_2:def 1;
  then B4: h in dom phi by B2;
  psi.g = psi.([.a,b.]) by B2
       .= [.psi.a,psi.b.] by GROUP_6:34
       .= h;
  then B5: psi.g = h;
  B6: psi is bijective;
  then B7: phi.h = phi.(psi.g) by B5
                .= phi.((phi").g)
                .= g by B6,\nwlinkedidentc{Th6}{NW18nMBU-1Brdpy-1};
  B8: phi.((phi").g) = g by \nwlinkedidentc{Th6}{NW18nMBU-1Brdpy-1};
  h in dom phi & h in commutators G by B3,B4;
  then B6: phi.h in phi .: commutators G by B1,FUNCT_1:def 6;
  then phi.(psi.g) in phi .: commutators G by B5;
  then phi.((phi").g) in phi .: commutators G;
  then g in phi .: commutators G by B8;
  hence thesis;
end;
...
proof
  ...
  proof
    let y be object;
    assume B1: y in rng(phi|H);
    then consider x being object such that
    B2: x in dom(phi|H) and
    B3: y = (phi|H).x
    by B1,FUNCT_1:def 3;
    dom (phi|H) = the carrier of H by FUNCT_2:def 1;
    then B4: x in H by B2;
    then reconsider x as Element of H;
    x is Element of G by GROUP_2:42;
    then (phi|H).x = phi.x by B4,\nwlinkedidentc{Th2}{NW18nMBU-32pfqx-1};
    then B5: y = phi.x by B3;
    phi.x in H by A1;
    hence y in the carrier of H by B5;
  end;
...
proof
  ...
  proof
    let phi be \nwlinkedidentc{Automorphism}{NW1Nnu7l-2qSYW4-1} of G;
    B1: dom phi = the carrier of G by FUNCT_2:def 1;
    ...
    proof
      ...
      C4: Product(F1 |^ I) = h
      by GROUP_4:28;
      phi.h = phi.(Product(F1 |^ I)) by C4;
      deffunc F(Nat) = phi.(F1/.$1); :: phi.((F1/.$1) |^ @(I/.$1));
      ...
      proof
        D0: for a being object st a in A holds a in G;
        D1: for y being object st y in rng F2 holds y in A
        proof
          let y be object;
          assume E1: y in rng F2;
          then consider k being object such that
          E2: k in dom F2 and
          ...
          E4: x = F1/.k by E2,C7,PARTFUN1:def 6, PARTFUN1:4;
          x in A &
          A c= the carrier of G & dom phi = the carrier of G by FUNCT_2:def 1;
          then E5: x in dom phi by STRUCT_0:def 6;
          y = F(k) by E2,E3,C6
           .= phi.(F1/.k)
           .= phi.(x) by E4;
          then y in A by A2; :: , E5, FUNCT_1:def 3;
          then E5: y is Element of A by A1, FUNCT_1:def 3;
          thus thesis by E5;
        end;
        ...
        proof
          ...
          assume E1: y in rng F2;
          then y in A by D1;
          hence y in the carrier of G by D0;
        end;
        hence rng F2 c= the carrier of G by TARSKI:def 3;
        thus rng F2 c= A by D2, TARSKI:def 3;
      end;
      then reconsider F2 as FinSequence of the carrier of G;
      F2 is FinSequence of the carrier of G & rng F2 c= A by C8;
      set h2 = Product(F2 |^ I);
      ...
      then C10: len F2 = len I & rng F2 c= A & Product(F2 |^ I) = phi.h
      by C4,C9,C8;
      :: then Product(F2 |^ I) in H by GROUP_4:28;
    ...
    proof
      ...
      C1: h in H;
      C2: h is Element of G by GROUP_2:42;
